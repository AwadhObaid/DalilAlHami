...
try {
    ...
    $migrationId = "20260806170000"
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $logRoot = Join-Path $resolvedRoot `
        "build_logs\phase08a_media_verify_$timestamp"
    ...
    $markerPath = Join-Path $resolvedRoot `
        "PHASE_08A1_MEDIA_FOUNDATION_PRIMARY_IMAGES.txt"
    @(
        "Dalil Al Hami - Phase 08A-1 media foundation"
        "Migration: $migrationId"
        "Verified at: $(Get-Date -Format o)"
        "Log: $combinedPath"
        "Storage: category-media, business-media, advertisements"
        "Images: category, logo, cover, expanded ad, compact ad"
        "Local cache schema: 8"
