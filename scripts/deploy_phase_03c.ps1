...
try {
    ...
    $migrationPath = Join-Path $resolvedProjectRoot `
        "supabase\migrations\20260802220000_phone_auth_owner_business.sql"

    if (-not (Test-Path -LiteralPath $migrationPath -PathType Leaf)) {
        throw "ملف Migration الخاص بالمرحلة 03C غير موجود."
    }
    ...
    try {
        ...
        Write-Host ""
        Write-Host "==> معاينة Migration المرحلة 03C" `
        ...
        $confirmation = Read-Host `
            "اكتب APPLY لتطبيق المرحلة 03C على Supabase"
        ...
        Write-Host ""
        Write-Host "==> تطبيق Migration المرحلة 03C" `
            -ForegroundColor Cyan
        ...
        Write-Host ""
        Write-Host "[ OK ] تم تطبيق قاعدة بيانات المرحلة 03C." `
