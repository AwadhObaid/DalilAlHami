...
try {
    ...
    $migrationPath = Join-Path $resolvedRoot `
        "supabase\migrations\20260804193000_sync_conflict_management.sql"
    $linkedRefPath = Join-Path $resolvedRoot "supabase\.temp\project-ref"

    if (-not (Test-Path -LiteralPath $migrationPath -PathType Leaf)) {
        throw "Phase 05B-3 migration was not found."
    }
    ...
    Write-Host ""
    Write-Host "==> Applying Phase 05B-3 Supabase migration" `
        -ForegroundColor Cyan
    Write-Host (
        "Choose Yes when Supabase asks to apply " +
        "20260804193000_sync_conflict_management.sql."
    ...
    Write-Host "[ OK ] Conflict management migration was pushed." `
