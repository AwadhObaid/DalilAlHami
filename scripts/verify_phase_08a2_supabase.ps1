...
try {
    ...
    $migrationId = "20260806230000"
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $logRoot = Join-Path $resolvedRoot `
        "build_logs\phase08a2_media_verify_$timestamp"
    ...
    $markerPath = Join-Path $resolvedRoot `
        "PHASE_08A2_STORAGE_OWNERSHIP_POLICY_FIX_V5.txt"
    @(
        "Dalil Al Hami - Phase 08A-2 storage ownership policy fix v5"
        "Migration: $migrationId"
        "Verified at: $(Get-Date -Format o)"
        "Log: $combinedPath"
        "Gallery limit: 5"
        "Final business media owner policies: enabled"
        "Profile avatar owner policies: enabled"
        "Profile avatar upload: enabled"
        "Admin cleanup revalidation: enabled"
        "Local cache schema: 9"
