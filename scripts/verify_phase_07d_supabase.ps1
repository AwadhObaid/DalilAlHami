...
try {
    ...
    $migrationId = "20260806133000"
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $logRoot = Join-Path $resolvedRoot `
        "build_logs\phase07d_verify_$timestamp"
    ...
    $markerPath = Join-Path $resolvedRoot `
        "PHASE_07D_ADMIN_ADVERTISEMENT_MANAGEMENT.txt"
    @(
        "Dalil Al Hami - Phase 07D advertisement management"
        "Migration: $migrationId"
        "Verified at: $(Get-Date -Format o)"
        "Log: $combinedPath"
        "Features: advertisement CRUD, activation, schedules, placements, targets"
        "Audit: public.admin_content_actions / entity_type advertisement"
        "Sync: placement-aware advertisement payload and local schema v7"
