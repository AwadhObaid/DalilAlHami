...
try {
    ...
    $migrationId = "20260807010000"
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $logRoot = Join-Path $resolvedRoot `
        "build_logs\phase08b_location_verify_$timestamp"
    ...
    $markerPath = Join-Path $resolvedRoot `
        "PHASE_08B_MAPS_LOCATION.txt"
    @(
        "Dalil Al Hami - Phase 08B maps and location"
        "Migration: $migrationId"
        "Verified at: $(Get-Date -Format o)"
        "Log: $combinedPath"
        "Map provider: OpenStreetMap"
        "Foreground location permission: enabled"
        "Owner location RPC: enabled"
        "Local cache schema: 10"
