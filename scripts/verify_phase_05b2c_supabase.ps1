...
try {
    ...
    $migrationId = "20260804174500"
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $logRoot = Join-Path $resolvedRoot `
        "build_logs\phase05b2c_verify_$timestamp"
    ...
    if ($matches.Count -lt 2) {
        throw "Migration $migrationId was not confirmed in LOCAL and REMOTE."
    }

    $markerPath = Join-Path $resolvedRoot `
        "PHASE_05B2C_SUPABASE_VERIFIED.txt"
    @(
        "Dalil Al Hami - Phase 05B-2C Supabase verified"
