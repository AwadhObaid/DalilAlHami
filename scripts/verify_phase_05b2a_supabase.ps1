...
try {
    ...
    $migrationId = "20260804003000"
    $logsRoot = Join-Path $resolvedRoot "build_logs"
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $logRoot = Join-Path $logsRoot "phase05b2a_verify_$timestamp"
    ...
    if ($process.ExitCode -ne 0) {
        throw "Supabase migration list failed with exit code $($process.ExitCode)."
    }
    ...
    if ($matches.Count -lt 2) {
        throw (
            "Migration $migrationId was not confirmed in both " +
            "LOCAL and REMOTE columns."
        )
    }

    $markerPath = Join-Path $resolvedRoot `
        "PHASE_05B2A_SUPABASE_VERIFIED.txt"
    @(
        "Dalil Al Hami - Phase 05B-2A Supabase verified"
        "Migration: $migrationId"
        "Verified at: $(Get-Date -Format o)"
        "Log: $combinedPath"
    ) | Set-Content -LiteralPath $markerPath -Encoding UTF8

    Write-Host "[ OK ] Migration $migrationId exists locally and remotely." `
        -ForegroundColor Green
    Write-Host "[ OK ] Verification log: $combinedPath" `
        -ForegroundColor Green
    Write-Host "[ OK ] Marker: $markerPath" `
        -ForegroundColor Green
}
catch {
