-- Dalil Al Hami
-- Phase 07D: secure advertisement management and placement-aware sync

begin;

alter table public.admin_content_actions
  drop constraint if exists admin_content_actions_entity_type_check;
alter table public.admin_content_actions
  add constraint admin_content_actions_entity_type_check
  check (entity_type in ('category', 'business', 'advertisement'));
...
create or replace function public.admin_set_advertisement_active(
  p_advertisement_id uuid,
  p_is_active boolean
)
returns jsonb
...
begin
  ...
  v_action := case when v_advertisement.is_active
    then 'activated'
    else 'deactivated'
  end;
  ...
  return jsonb_build_object(
    ...
    'entity_type', 'advertisement',
    'action', v_action,
    'message', case when v_advertisement.is_active
      then 'تم تفعيل الإعلان.'
      else 'تم إيقاف الإعلان.'
    end
  );
end;
$$;

create or replace function public.admin_delete_advertisement(
  p_advertisement_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  return jsonb_build_object(
    'entity_id', p_advertisement_id,
    'entity_type', 'advertisement',
    'action', 'deleted',
    'message', 'تم حذف الإعلان نهائيًا.'
  );
end;
$$;
...
revoke all on function public.admin_set_advertisement_active(uuid, boolean)
  from public;
revoke all on function public.admin_delete_advertisement(uuid) from public;
...
grant execute on function public.admin_set_advertisement_active(uuid, boolean)
  to authenticated;
grant execute on function public.admin_delete_advertisement(uuid)
  to authenticated;
grant execute on function public.get_directory_changes(bigint)
  to anon, authenticated;

comment on table public.admin_content_actions is
  'Immutable administrator audit trail for category, business, and advertisement management.';
comment on function public.admin_upsert_advertisement(
  uuid, uuid, text, text, text, text, timestamptz, timestamptz, integer
) is 'Creates or updates an advertisement after verifying administrator access.';
